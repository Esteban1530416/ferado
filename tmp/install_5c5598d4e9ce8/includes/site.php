...
class SunFwSite
{
	...
	public function optimizeAssets()
	{
		...
		// Get list of module that will be loaded on the requested page.
		$modules = array();

		foreach (SunFwHelper::getModulePositions($this->template) as $position)
		{
			$modules = array_merge($modules, JModuleHelper::getModules($position));
		}

		// Load additional assets if has for the requested component and loaded modules.
		$base_path = JPATH_THEMES . "/{$this->template}";
		$base_link = JUri::root(true) . "/templates/{$this->template}";

		foreach (array_merge((array) $this->app->input->getCmd('option'), $modules) as $extension)
		{
			...
			{
				if (is_file("{$base_path}/{$type}/{$extension}.{$type}"))
				{
					if ($type === 'css')
					{
						$this->doc->addStyleSheet("{$base_link}/{$type}/{$extension}.{$type}");
					}
					else
					{
						$this->doc->addScript("{$base_link}/{$type}/{$extension}.{$type}");
					}
				}

				// If the current extension is a component, check if there is an asset file for the requested view.
				if (strpos($extension, 'com_') === 0 && is_dir("{$base_path}/{$type}/{$extension}"))
				{
					$view = $this->app->input->getCmd('view');

					if (is_file("{$base_path}/{$type}/{$extension}/{$view}.{$type}"))
					{
						if ($type === 'css')
						{
							$this->doc->addStyleSheet("{$base_link}/{$type}/{$extension}/{$view}.{$type}");
						}
						else
						{
							$this->doc->addScript("{$base_link}/{$type}/{$extension}/{$view}.{$type}");
						}
					}
				}
			}
