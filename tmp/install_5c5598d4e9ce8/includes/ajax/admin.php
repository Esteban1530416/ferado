...
class SunFwAjaxAdmin extends SunFwAjax
{
	...
	public function getAction()
	{
		...
		$ajaxBase = "{$root}administrator/index.php?option=com_ajax&format=json&group=system&plugin=sunfw&template_name={$this->templateName}&style_id={$this->styleID}&" .
		...
		$info = SunFwHttp::get(
			SUNFW_GET_INFO_URL . "&token={$params['token']}&identified_name=" . SUNFW_ID,
			3600
		);
		...
		$this->setResponse(
			array(
				...
				'ga' => array(
					...
					'profile' => (isset($info) && is_array($info['message'])) ? $info['message']['ga_property_id'] : null,
					'set' => array(
	...
	protected function getComponentBody()
	{
		...
		$saveAsCopy = "index.php?option=com_ajax&format=json&group=system&plugin=sunfw&context=admin&action=saveAsCopy&template_name={$this->templateName}&style_id={$this->styleID}&" .
			 JSession::getFormToken() . '=1';
	...
	{
		$ajaxServer = "index.php?option=com_ajax&format=json&group=system&plugin=sunfw&context=update&&style_id={$this->styleID}&template_name={$this->templateName}&" .
			 JSession::getFormToken() . '=1&';
