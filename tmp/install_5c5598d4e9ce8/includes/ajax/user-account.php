...
class SunFwAjaxUserAccount extends SunFwAjax
{
	...
	public function getTokenKeyAction()
	{
		...
		try
		{
			$result = SunFwHttp::post(SUNFW_GET_TOKEN_URL, http_build_query($query, null, '&'));
	...
	{
		// Clear cached license data.
		$cache = JFactory::getConfig()->get('tmp_path') . "/{$this->template['id']}/license";
	...
	{
		// Clear username and token data for the template record.
		SunFwHelper::updateExtensionParams(array(
			'username' => null,
			'token' => null
		), 'template', $this->template['name']);

		// Clear token data for the Sun Framework record also.
		SunFwHelper::updateExtensionParams(array(
			'username' => null,
			'token' => null
		), 'plugin', 'sunfw', 'system');
